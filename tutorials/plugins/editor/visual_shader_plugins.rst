...
Plugin Visual Shader được dùng để tạo các node :ref:`class_VisualShader` tùy chỉnh trong GDScript.

Quy trình tạo khác với các plugin editor thông thường. Bạn không cần tạo tệp ``plugin.cfg`` để đăng ký plugin; thay vào đó, hãy tạo và lưu một tệp script, sau đó có thể sử dụng ngay, miễn là node tùy chỉnh được đăng ký bằng ``class_name``.

Bài hướng dẫn ngắn này sẽ giải thích cách tạo một node nhiễu Perlin-3D (mã nguồn gốc từ `GPU noise shaders plugin <https://github.com/curly-brace/Godot-3.0-Noise-Shaders/blob/master/assets/gpu_noise_shaders/classic_perlin3d.tres>`_.

Tạo một Sprite2D và gán :ref:`class_ShaderMaterial` vào ô material của nó:
...
::
    ...
    func _get_name():
        ...
        # Tên phải bắt đầu bằng một chữ cái và *không* được chứa khoảng trắng.
        return "PerlinNoise3D"


    func _get_category():
        # Đây phải là một identifier hợp lệ và nên tuân theo quy ước đặt tên PascalCase.
        # Category phải bắt đầu bằng một chữ cái và *không* được chứa khoảng trắng.
    ...
    func _get_global_code(_mode):
        return """
            ...
            float cnoise(vec3 P) {
                vec3 Pi0 = floor(P); // Phần nguyên để indexing.
...
Lưu lại và mở Visual Shader. Bạn sẽ thấy loại node mới trong hộp thoại member, bên dưới category Addons (nếu không thấy node mới, hãy thử khởi động lại editor):

.. image:: img/visual_shader_plugins_result1.png

Đặt nó trên graph và kết nối các port bắt buộc:

.. image:: img/visual_shader_plugins_result2.png

Đó là tất cả những gì bạn cần làm; như bạn thấy, việc tạo các node VisualShader tùy chỉnh của riêng mình thật dễ dàng!
