...
Hỗ trợ các trạng thái di chuyển khác nhau của actor
===================================================

.. image:: img/nav_actor_locomotion.png

Để hỗ trợ các trạng thái di chuyển khác nhau của actor, chẳng hạn như cúi người và bò, cần có thiết lập map tương tự như khi hỗ trợ :ref:`doc_navigation_different_actor_types`.

Bake các navigation mesh khác nhau với chiều cao phù hợp cho actor đang cúi hoặc bò, để chúng có thể tìm đường đi qua những khu vực hẹp đó trong thế giới game của bạn.

Khi actor thay đổi trạng thái di chuyển, chẳng hạn như đứng lên, bắt đầu cúi hoặc bò, hãy truy vấn map phù hợp để tìm đường đi.

Nếu hành vi avoidance cũng cần thay đổi theo trạng thái di chuyển, chẳng hạn như chỉ tránh khi đang đứng hoặc chỉ tránh các agent khác đang ở cùng trạng thái di chuyển, hãy chuyển avoidance agent của actor sang một avoidance map khác sau mỗi lần thay đổi trạng thái di chuyển.
...
.. note::
    ...
    Mặc dù có thể thực thi truy vấn đường đi ngay lập tức cho nhiều map, việc chuyển map của avoidance agent chỉ có hiệu lực sau lần đồng bộ máy chủ tiếp theo.
