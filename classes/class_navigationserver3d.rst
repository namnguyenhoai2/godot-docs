...
.. KHÔNG ĐƯỢC CHỈNH SỬA TỆP NÀY!!! .. Được tự động tạo từ mã nguồn engine Godot. .. Generator: https://github.com/godotengine/godot/tree/master/doc/tools/make_rst.py. .. XML source: https://github.com/godotengine/godot/tree/master/doc/classes/NavigationServer3D.xml.
...
Một server interface để truy cập navigation 3D ở mức thấp.
...
NavigationServer3D là server xử lý các navigation map, region và agent. Nó không xử lý navigation A\* từ :ref:`AStar3D<class_AStar3D>`.

Các map được chia thành các region, vốn được cấu thành từ navigation mesh. Kết hợp lại, chúng xác định các khu vực có thể di chuyển trong thế giới 3D.

\ **Lưu ý:** Hầu hết thay đổi của **NavigationServer3D** có hiệu lực sau frame vật lý tiếp theo, không phải ngay lập tức. Điều này bao gồm mọi thay đổi đối với map, region hoặc agent do các node liên quan đến navigation trong scene tree thực hiện hoặc được thực hiện thông qua script.

Để hai region được kết nối với nhau, chúng phải có một cạnh tương tự nhau. Một cạnh được xem là kết nối với cạnh khác nếu cả hai đỉnh của nó đều cách đỉnh tương ứng của cạnh kia một khoảng nhỏ hơn ``edge_connection_margin``.

Bạn có thể gán navigation layer cho các region bằng :ref:`region_set_navigation_layers()<class_NavigationServer3D_method_region_set_navigation_layers>`, sau đó kiểm tra chúng khi yêu cầu một path bằng :ref:`map_get_path()<class_NavigationServer3D_method_map_get_path>`. Điều này có thể được dùng để cho phép hoặc từ chối một số khu vực đối với một số đối tượng.

Để sử dụng hệ thống collision avoidance, bạn có thể dùng agent. Bạn có thể đặt target velocity cho agent, sau đó các server sẽ phát callback với một velocity đã được điều chỉnh.

\ **Lưu ý:** Hệ thống collision avoidance bỏ qua các region. Việc sử dụng trực tiếp velocity đã được điều chỉnh có thể khiến agent di chuyển ra ngoài khu vực có thể đi qua. Đây là một hạn chế của hệ thống collision avoidance; những tình huống phức tạp hơn có thể yêu cầu sử dụng physics engine.

Server này lưu lại mọi lệnh gọi và thực thi chúng trong sync phase. Điều này có nghĩa là bạn có thể yêu cầu bất kỳ thay đổi nào đối với map từ bất kỳ thread nào mà không cần lo lắng.
...
Được phát khi các thiết lập debug avoidance thay đổi. Chỉ khả dụng trong các bản build debug.
...
Được phát khi một navigation map được cập nhật, khi một region di chuyển hoặc được sửa đổi.
...
Được phát khi các thiết lập debug navigation thay đổi. Chỉ khả dụng trong các bản build debug.
...
Hằng số dùng để lấy số lượng navigation map đang hoạt động.
...
Hằng số dùng để lấy số lượng navigation region đang hoạt động.
...
Hằng số dùng để lấy số lượng navigation agent đang xử lý avoidance.
...
Hằng số dùng để lấy số lượng navigation link đang hoạt động.
...
Hằng số dùng để lấy số lượng polygon của navigation mesh.
...
Hằng số dùng để lấy số lượng cạnh của polygon navigation mesh.
...
Hằng số dùng để lấy số lượng cạnh của polygon navigation mesh đã được gộp do các edge key chồng lấp.
...
Hằng số dùng để lấy số lượng cạnh của polygon navigation mesh được xem là đã kết nối dựa trên khoảng cách giữa các cạnh.
...
Hằng số dùng để lấy số lượng cạnh của polygon navigation mesh không thể được gộp nhưng vẫn có thể được kết nối dựa trên khoảng cách giữa các cạnh hoặc với các link.
...
Hằng số dùng để lấy số lượng navigation obstacle đang hoạt động.
...
Trả về ``true`` nếu ``agent`` được cung cấp đã bật avoidance.
...
Trả về ``height`` của ``agent`` được chỉ định.
...
Trả về khoảng thời gian tối thiểu mà trong đó các velocity của ``agent`` được chỉ định, vốn được simulation tính toán, an toàn đối với các agent khác.
...
Trả về khoảng thời gian tối thiểu mà trong đó các velocity của ``agent`` được chỉ định, vốn được simulation tính toán, an toàn đối với các avoidance obstacle tĩnh.
...
Trả về ``true`` nếu ``agent`` được cung cấp sử dụng avoidance trong không gian 3D Vector3(x,y,z) thay vì Vector2(x,y) 2D nằm ngang / Vector3(x,0.0,z).
...
Trả về velocity của ``agent`` được chỉ định.
...
Trả về ``true`` nếu ``agent`` được chỉ định có avoidance callback.
...
Thiết lập callback :ref:`Callable<class_Callable>` được gọi sau mỗi bước xử lý tránh va chạm cho ``agent``. ``safe_velocity`` được tính toán sẽ được gửi kèm một signal đến đối tượng ngay trước các phép tính vật lý.

\ **Lưu ý:** Các callback đã tạo luôn được xử lý độc lập với trạng thái SceneTree miễn là agent nằm trên một navigation map và chưa được giải phóng. Để tắt việc gửi callback từ một agent, hãy sử dụng lại :ref:`agent_set_avoidance_callback()<class_NavigationServer3D_method_agent_set_avoidance_callback>` với :ref:`Callable<class_Callable>` rỗng.
...
Nếu ``enabled`` là ``true``, ``agent`` được cung cấp sẽ tính toán việc tránh va chạm.
...
Thiết lập ``avoidance_priority`` của agent với ``priority`` trong khoảng từ 0.0 (độ ưu tiên thấp nhất) đến 1.0 (độ ưu tiên cao nhất).

``agent`` được chỉ định sẽ không điều chỉnh velocity cho các agent khác khớp với ``avoidance_mask`` nhưng có ``avoidance_priority`` thấp hơn. Do đó, các agent khác có độ ưu tiên thấp hơn sẽ điều chỉnh velocity của chúng nhiều hơn để tránh va chạm với agent này.
...
Thiết lập số lượng tối đa các agent khác mà agent này xét đến trong navigation. Con số này càng lớn thì thời gian chạy của simulation càng lâu. Nếu con số quá thấp, simulation sẽ không an toàn.
...
Thiết lập speed tối đa của agent. Giá trị phải dương.
...
Thiết lập khoảng cách tối đa đến các agent khác mà agent này xét đến trong navigation. Con số này càng lớn thì thời gian chạy của simulation càng lâu. Nếu con số quá thấp, simulation sẽ không an toàn.
...
Nếu ``paused`` là ``true``, ``agent`` được chỉ định sẽ không được xử lý. Ví dụ: nó sẽ không tính toán avoidance velocity hoặc nhận avoidance callback.
...
Khoảng thời gian tối thiểu mà velocity của agent, được simulation tính toán, an toàn đối với các agent khác. Con số này càng lớn thì agent càng sớm phản ứng với sự hiện diện của các agent khác, nhưng agent càng ít tự do trong việc chọn velocity. Giá trị quá cao sẽ làm chuyển động của các agent chậm đi đáng kể. Giá trị phải dương.
...
Khoảng thời gian tối thiểu mà velocity của agent, được simulation tính toán, an toàn đối với các static avoidance obstacle. Con số này càng lớn thì agent càng sớm phản ứng với sự hiện diện của các static avoidance obstacle, nhưng agent càng ít tự do trong việc chọn velocity. Giá trị quá cao sẽ làm chuyển động của các agent chậm đi đáng kể. Giá trị phải dương.
...
Thiết lập agent sử dụng 2D avoidance hay 3D avoidance khi avoidance được bật.

Nếu ``true``, agent sẽ tính toán avoidance velocity trong 3D theo các trục xyz, ví dụ như trong các game diễn ra trên không, dưới nước hoặc ngoài không gian. Agent sử dụng 3D chỉ tránh các agent khác cũng sử dụng 3D avoidance. Agent sử dụng 3D chỉ phản ứng với các avoidance obstacle dựa trên radius. Agent sử dụng 3D bỏ qua mọi obstacle dựa trên vertex. Agent sử dụng 3D chỉ tránh các agent khác cũng sử dụng 3D.

Nếu ``false``, agent sẽ tính toán avoidance velocity trong 2D dọc theo trục xz và bỏ qua trục y. Agent sử dụng 2D chỉ tránh các agent khác cũng sử dụng 2D avoidance. Agent sử dụng 2D phản ứng với các avoidance obstacle dựa trên radius. Agent sử dụng 2D phản ứng với các avoidance obstacle dựa trên vertex. Agent sử dụng 2D chỉ tránh các agent khác cũng sử dụng 2D. Các agent sử dụng 2D sẽ bỏ qua các agent sử dụng 2D khác hoặc các obstacle nằm dưới hoặc trên vị trí hiện tại của chúng, bao gồm cả chiều cao của agent trong 2D avoidance.
...
Thiết lập ``velocity`` làm velocity mong muốn mới cho ``agent`` được chỉ định. Simulation avoidance sẽ cố gắng đáp ứng velocity này nếu có thể, nhưng sẽ điều chỉnh nó để tránh va chạm với các agent khác và obstacle. Khi một agent được dịch chuyển tức thời đến vị trí mới, hãy sử dụng cả :ref:`agent_set_velocity_forced()<class_NavigationServer3D_method_agent_set_velocity_forced>` để đặt lại velocity nội bộ của simulation.
...
Thay thế velocity nội bộ trong collision avoidance simulation bằng ``velocity`` cho ``agent`` được chỉ định. Khi một agent được dịch chuyển tức thời đến vị trí mới, nên sử dụng function này trong cùng frame. Nếu được gọi thường xuyên, function này có thể khiến các agent bị mắc kẹt.
...
Bake ``navigation_mesh`` được cung cấp bằng dữ liệu từ ``source_geometry_data`` được cung cấp. Sau khi quá trình hoàn tất, ``callback`` tùy chọn sẽ được gọi.
...
Bake ``navigation_mesh`` được cung cấp bằng dữ liệu từ ``source_geometry_data`` được cung cấp dưới dạng một async task chạy trên background thread. Sau khi quá trình hoàn tất, ``callback`` tùy chọn sẽ được gọi.
...
Trả về tất cả :ref:`RID<class_RID>`\ s của navigation map đã tạo trên NavigationServer. Hàm này trả về cả navigation map 2D và 3D đã tạo vì về mặt kỹ thuật không có sự phân biệt giữa chúng.
...
Trả về ``true`` khi navigation mesh được cung cấp đang được bake trên background thread.
...
Trả về ID lần lặp hiện tại của liên kết điều hướng. Mỗi khi liên kết điều hướng thay đổi và đồng bộ hóa, ID lần lặp sẽ tăng. ID lần lặp ``0`` có nghĩa là liên kết điều hướng chưa từng được đồng bộ hóa.

\ **Lưu ý:** ID lần lặp sẽ quay vòng về ``1`` sau khi đạt đến giới hạn phạm vi của nó.
...
Thiết lập liệu ``link`` này có thể được di chuyển theo cả hai hướng hay không.
...
Thiết lập navigation map :ref:`RID<class_RID>` cho liên kết.
...
Thiết lập navigation layer của liên kết. Điều này cho phép chọn các liên kết từ một yêu cầu đường đi (khi sử dụng :ref:`map_get_path()<class_NavigationServer3D_method_map_get_path>`).
...
Thiết lập ``ObjectID`` của đối tượng quản lý liên kết này.
...
**Không còn được khuyến nghị:** Phương thức này không còn được hỗ trợ vì không tương thích với các bản cập nhật bất đồng bộ. Chỉ có thể sử dụng phương thức này trong ngữ cảnh đơn luồng, và bạn tự chịu rủi ro khi sử dụng.

Hàm này lập tức buộc đồng bộ hóa navigation ``map`` được chỉ định :ref:`RID<class_RID>`. Theo mặc định, navigation map chỉ được đồng bộ hóa ở cuối mỗi khung hình vật lý. Có thể sử dụng hàm này để lập tức tính toán (hoặc tính toán lại) tất cả navigation mesh và kết nối region của navigation map. Nhờ đó, có thể truy vấn đường dẫn điều hướng cho một map đã thay đổi ngay lập tức và trong cùng một khung hình (nhiều lần nếu cần).

Do các hạn chế kỹ thuật, hàng đợi lệnh NavigationServer hiện tại sẽ được xả. Điều này có nghĩa là tất cả lệnh cập nhật đã được xếp hàng cho khung hình vật lý này sẽ được thực thi, kể cả những lệnh dành cho các map, region và agent khác không thuộc map được chỉ định. Phép tính tốn kém cho navigation mesh và kết nối region của một map chỉ được thực hiện đối với map được chỉ định. Các map khác sẽ nhận được lần đồng bộ hóa thông thường ở cuối khung hình vật lý. Nếu map được chỉ định nhận thay đổi sau lần cập nhật bắt buộc, map đó cũng sẽ được cập nhật lại khi các map khác nhận bản cập nhật của chúng.

Việc xử lý tránh né và gửi các tín hiệu ``safe_velocity`` không bị ảnh hưởng bởi hàm này và vẫn tiếp tục diễn ra cho tất cả map và agent ở cuối khung hình vật lý.

\ **Lưu ý:** Quyền lực lớn đi kèm với trách nhiệm lớn. Chỉ những người dùng thực sự hiểu rõ mình đang làm gì và có lý do chính đáng mới nên sử dụng hàm này. Việc buộc cập nhật ngay lập tức một navigation map yêu cầu khóa NavigationServer và xả toàn bộ hàng đợi lệnh của NavigationServer. Điều này không chỉ có thể ảnh hưởng nghiêm trọng đến hiệu năng của trò chơi mà còn có thể gây ra lỗi nếu được sử dụng không phù hợp và thiếu cân nhắc.
...
Trả về tất cả agent điều hướng :ref:`RID<class_RID>`\ s hiện đang được gán vào navigation ``map`` được yêu cầu.
...
Trả về chiều cao ô của map được sử dụng để rasterize các đỉnh của navigation mesh trên trục Y.
...
Trả về kích thước ô của map được sử dụng để rasterize các đỉnh của navigation mesh trên mặt phẳng XZ.
...
Trả về điểm trên bề mặt navigation mesh gần nhất với ``to_point`` được cung cấp trên navigation ``map``.
...
Trả về pháp tuyến bề mặt navigation mesh gần nhất với ``to_point`` được cung cấp trên navigation ``map``.
...
Trả về RID của region sở hữu điểm trên bề mặt navigation mesh gần nhất với ``to_point`` được cung cấp trên navigation ``map``.
...
Trả về điểm trên bề mặt navigation mesh gần nhất với ``start`` và đoạn ``end`` được cung cấp trên navigation ``map``.

Nếu ``use_collision`` là ``true``, chỉ thực hiện kiểm tra điểm gần nhất khi đoạn đó giao với bề mặt navigation mesh.
...
Trả về margin kết nối cạnh của map. Khoảng cách này là khoảng cách tối thiểu giữa các đỉnh cần thiết để kết nối hai cạnh từ các region khác nhau.
...
Trả về id iteration hiện tại của navigation map. Mỗi khi navigation map thay đổi và đồng bộ hóa, id iteration sẽ tăng. Id iteration bằng 0 nghĩa là navigation map chưa từng được đồng bộ hóa.

\ **Lưu ý:** Id iteration sẽ quay lại 1 sau khi đạt đến giới hạn phạm vi.
...
Trả về bán kính kết nối link của map. Khoảng cách này là phạm vi tối đa mà mọi link sẽ tìm kiếm các polygon của navigation mesh để kết nối.
...
Trả về tất cả :ref:`RID<class_RID>`\ s link điều hướng hiện được gán cho ``map`` điều hướng được yêu cầu.
...
Trả về cell scale của merge rasterizer nội bộ của map.
...
Trả về tất cả :ref:`RID<class_RID>`\ s obstacle điều hướng hiện được gán cho ``map`` điều hướng được yêu cầu.
...
Trả về navigation path để đi từ origin đến destination. ``navigation_layers`` là bitmask của tất cả navigation layer của region được phép xuất hiện trong path.
...
Trả về một vị trí ngẫu nhiên được chọn từ tất cả polygon của region trong map khớp với ``navigation_layers``.
...
Trả về tất cả :ref:`RID<class_RID>`\ s region điều hướng hiện được gán cho ``map`` điều hướng được yêu cầu.
...
Trả về ``true`` nếu ``map`` điều hướng cho phép các region điều hướng sử dụng edge connection để kết nối với các region điều hướng khác nằm trong phạm vi gần margin edge connection của navigation map.
...
Đặt cell height của map được dùng để rasterize các vertex của navigation mesh trên trục Y. Phải khớp với cell height của các navigation mesh được sử dụng.
...
Đặt cell size của map được dùng để rasterize các vertex của navigation mesh trên mặt phẳng XZ. Phải khớp với cell size của các navigation mesh được sử dụng.
...
Đặt margin edge connection của map được dùng để hàn các edge tương thích của region.
...
Đặt bán kính kết nối link của map được dùng để kết nối các link với các polygon điều hướng.
...
Đặt cell scale của merge rasterizer nội bộ của map, được dùng để kiểm soát độ nhạy khi hợp nhất.
...
Nếu ``enabled`` là ``true``, quá trình đồng bộ hóa ``map`` sử dụng một quy trình async chạy trên background thread.
...
Đặt việc sử dụng edge connection của ``map`` điều hướng. Nếu ``enabled`` là ``true``, navigation map cho phép các region điều hướng sử dụng edge connection để kết nối với các region điều hướng khác nằm trong phạm vi gần margin edge connection của navigation map.
...
Trả về ``true`` nếu ``obstacle`` được cung cấp đã bật avoidance.
...
Trả về ``height`` của ``obstacle`` được chỉ định.
...
Trả về bán kính của ``obstacle`` dynamic được chỉ định.
...
Trả về ``true`` nếu ``obstacle`` được cung cấp sử dụng avoidance trong không gian 3D Vector3(x,y,z) thay vì không gian 2D ngang Vector2(x,y) / Vector3(x,0.0,z).
...
Trả về velocity của ``obstacle`` dynamic được chỉ định.
...
Trả về các vertex đường bao của ``obstacle`` được chỉ định.
...
Nếu ``enabled`` là ``true``, ``obstacle`` được cung cấp sẽ ảnh hưởng đến avoidance bằng cách sử dụng các agent.
...
Thiết lập bitmask ``avoidance_layers`` của obstacle.
...
Thiết lập ``height`` cho ``obstacle``. Trong 3D, các agent sẽ bỏ qua những obstacle nằm phía trên hoặc phía dưới chúng khi sử dụng 2D avoidance.
...
Gán ``obstacle`` vào một navigation map.
...
Nếu ``paused`` là ``true``, ``obstacle`` được chỉ định sẽ không được xử lý. Ví dụ, nó sẽ không còn ảnh hưởng đến các vận tốc avoidance.
...
Cập nhật ``position`` trong world space cho ``obstacle``.
...
Thiết lập bán kính của dynamic obstacle.
...
Thiết lập liệu ``obstacle`` sử dụng 2D avoidance hay 3D avoidance khi avoidance được bật.
...
Thiết lập ``velocity`` của dynamic ``obstacle``. Cho phép các agent khác dự đoán tốt hơn chuyển động của dynamic obstacle. Chỉ hoạt động khi được kết hợp với bán kính của obstacle.
...
Thiết lập các đỉnh đường viền cho obstacle. Nếu các vertices được sắp theo thứ tự clockwise, các agent sẽ bị obstacle đẩy vào trong; nếu không, chúng sẽ bị đẩy ra ngoài.
...
Phân tích :ref:`SceneTree<class_SceneTree>` cho source geometry theo các thuộc tính của ``navigation_mesh``. Cập nhật resource ``source_geometry_data`` được cung cấp bằng dữ liệu thu được. Sau đó có thể sử dụng resource này để bake navigation mesh bằng :ref:`bake_from_source_geometry_data()<class_NavigationServer3D_method_bake_from_source_geometry_data>`. Sau khi quá trình hoàn tất, ``callback`` tùy chọn sẽ được gọi.

\ **Lưu ý:** Hàm này cần chạy trên main thread hoặc bằng deferred call vì SceneTree không an toàn khi chạy đa luồng.

\ **Hiệu năng:** Mặc dù thuận tiện, việc đọc các mảng dữ liệu từ resource :ref:`Mesh<class_Mesh>` có thể ảnh hưởng tiêu cực đến frame rate. Dữ liệu cần được nhận từ GPU, khiến :ref:`RenderingServer<class_RenderingServer>` bị đình trệ trong quá trình này. Để đạt hiệu năng tốt hơn, hãy ưu tiên sử dụng, chẳng hạn, collision shapes hoặc tạo hoàn toàn các mảng dữ liệu bằng code.
...
Truy vấn một path trong navigation map được cung cấp. Vị trí bắt đầu, vị trí đích và các tham số khác được định nghĩa thông qua :ref:`NavigationPathQueryParameters3D<class_NavigationPathQueryParameters3D>`. Cập nhật đối tượng kết quả :ref:`NavigationPathQueryResult3D<class_NavigationPathQueryResult3D>` được cung cấp bằng path cùng các kết quả khác được yêu cầu bởi truy vấn. Sau khi quá trình hoàn tất, ``callback`` tùy chọn sẽ được gọi.
...
**Đã lỗi thời:** Method này đã lỗi thời do các thay đổi về core threading. Để nâng cấp code hiện có, trước tiên hãy tạo một resource :ref:`NavigationMeshSourceGeometryData3D<class_NavigationMeshSourceGeometryData3D>`. Sử dụng resource này với :ref:`parse_source_geometry_data()<class_NavigationServer3D_method_parse_source_geometry_data>` để phân tích :ref:`SceneTree<class_SceneTree>` nhằm tìm các node cần đóng góp vào quá trình bake navigation mesh. Việc phân tích :ref:`SceneTree<class_SceneTree>` phải diễn ra trên main thread. Sau khi phân tích hoàn tất, hãy sử dụng resource này với :ref:`bake_from_source_geometry_data()<class_NavigationServer3D_method_bake_from_source_geometry_data>` để bake navigation mesh.

Bake ``navigation_mesh`` bằng source geometry được thu thập bắt đầu từ ``root_node``.
...
Trả về bounding box axis-aligned của navigation mesh đã được biến đổi của ``region``.
...
Trả về điểm trên bề mặt navigation mesh gần ``to_point`` được cung cấp nhất trên ``region`` navigation.
...
Trả về normal trên bề mặt navigation mesh gần ``to_point`` được cung cấp nhất trên ``region`` navigation.
...
Trả về điểm trên bề mặt navigation mesh gần đoạn từ ``start`` đến ``end`` được cung cấp nhất trên ``region`` navigation.

Nếu ``use_collision`` là ``true``, chỉ thực hiện kiểm tra điểm gần nhất khi đoạn thẳng giao với bề mặt navigation mesh.
...
Trả về điểm kết thúc của một connection door. ``connection`` là một chỉ mục nằm trong khoảng từ 0 đến giá trị trả về của :ref:`region_get_connections_count()<class_NavigationServer3D_method_region_get_connections_count>`.
...
Trả về điểm bắt đầu của một connection door. ``connection`` là một chỉ mục nằm trong khoảng từ 0 đến giá trị trả về của :ref:`region_get_connections_count()<class_NavigationServer3D_method_region_get_connections_count>`.
...
Trả về số lượng connection mà ``region`` này có với các region khác trong map.
...
Trả về iteration ID hiện tại của navigation region. Mỗi khi navigation region thay đổi và đồng bộ hóa, iteration ID sẽ tăng. Iteration ID bằng ``0`` nghĩa là navigation region chưa từng được đồng bộ hóa.

\ **Lưu ý:** Iteration ID sẽ quay vòng về ``1`` sau khi đạt đến giới hạn phạm vi.
...
Trả về navigation map :ref:`RID<class_RID>` mà ``region`` được yêu cầu hiện đang được gán vào.
...
Trả về các navigation layers của region.
...
Nếu ``uniformly`` là ``true``, tất cả polygon và face của region được tính trọng số theo diện tích bề mặt của chúng (chậm hơn).
...
Trả về ``true`` nếu ``region`` sử dụng một quy trình đồng bộ hóa async chạy trên background thread.
...
Trả về ``true`` nếu ``region`` điều hướng được thiết lập để sử dụng các kết nối cạnh nhằm kết nối với những navigation region khác nằm trong phạm vi lề kết nối cạnh của navigation map.
...
Trả về ``true`` nếu ``point`` được cung cấp trong world space hiện do navigation ``region`` được cung cấp sở hữu. Trong ngữ cảnh này, sở hữu có nghĩa là một trong các mặt polygon của navigation mesh thuộc region có một vị trí khả dĩ ở khoảng cách gần nhất với điểm này so với tất cả navigation mesh khác thuộc các navigation region khác cũng đã được đăng ký trên navigation map của region được cung cấp.
...
\ **Lưu ý:** Nếu các navigation mesh từ những navigation region khác nhau chồng lấp (điều này nhìn chung nên tránh), kết quả có thể không như mong đợi.
...
Thiết lập các navigation layer của region. Điều này cho phép chọn các region từ một yêu cầu tìm đường (khi sử dụng :ref:`map_get_path()<class_NavigationServer3D_method_map_get_path>`).
...
Thiết lập navigation mesh cho region.
...
Nếu ``enabled`` là ``true``, ``region`` sử dụng một quy trình đồng bộ hóa async chạy trên background thread.
...
Nếu ``enabled`` là ``true``, ``region`` điều hướng sẽ sử dụng các kết nối cạnh để kết nối với những navigation region khác nằm trong phạm vi lề kết nối cạnh của navigation map.
...
Điều khiển trạng thái kích hoạt của server này.
...
Nếu ``true`` được bật, debug mode sẽ được bật trên NavigationServer.
...
Trả về phiên bản đơn giản hóa của ``path``, trong đó các điểm đường đi ít quan trọng hơn đã được loại bỏ. Mức độ đơn giản hóa được tính theo đơn vị world và được điều khiển bởi ``epsilon``. Quá trình đơn giản hóa sử dụng một biến thể của thuật toán Ramer-Douglas-Peucker để giảm số điểm của đường cong.

Đơn giản hóa đường đi có thể giúp giảm thiểu nhiều vấn đề khi đi theo đường đi, vốn có thể phát sinh với một số loại agent và hành vi script nhất định. Ví dụ: agent "steering" hoặc cơ chế tránh trong "open fields".
...
Tạo một source geometry parser mới. Nếu một :ref:`Callable<class_Callable>` được thiết lập cho parser bằng :ref:`source_geometry_parser_set_callback()<class_NavigationServer3D_method_source_geometry_parser_set_callback>`, callback sẽ được gọi cho từng node được phân tích mỗi khi :ref:`parse_source_geometry_data()<class_NavigationServer3D_method_parse_source_geometry_data>` được sử dụng.
...
Thiết lập ``callback`` :ref:`Callable<class_Callable>` cho ``parser`` source geometry cụ thể. :ref:`Callable<class_Callable>` sẽ nhận một lời gọi với các tham số sau:

- ``navigation_mesh`` - Tham chiếu :ref:`NavigationMesh<class_NavigationMesh>` được sử dụng để xác định các thiết lập phân tích. KHÔNG chỉnh sửa hoặc thêm trực tiếp vào navigation mesh.

- ``source_geometry_data`` - Tham chiếu :ref:`NavigationMeshSourceGeometryData3D<class_NavigationMeshSourceGeometryData3D>`. Thêm source geometry tùy chỉnh để baking navigation mesh vào đối tượng này.

- ``node`` - :ref:`Node<class_Node>` được phân tích.
