...
.. KHÔNG ĐƯỢC CHỈNH SỬA TỆP NÀY!!! .. Được tự động tạo từ mã nguồn của Godot engine. .. Generator: https://github.com/godotengine/godot/tree/master/doc/tools/make_rst.py. .. XML source: https://github.com/godotengine/godot/tree/master/doc/classes/PackedInt64Array.xml.
...
Một mảng đóng gói gồm các số nguyên 64-bit.
...
Một mảng được thiết kế chuyên biệt để chứa các giá trị số nguyên 64-bit. Dữ liệu được đóng gói chặt chẽ, nhờ đó tiết kiệm bộ nhớ khi kích thước mảng lớn.

\ **Lưu ý:** Kiểu này lưu các số nguyên 64-bit có dấu, nghĩa là nó có thể nhận các giá trị trong khoảng ``[-2^63, 2^63 - 1]``, tức là ``[-9223372036854775808, 9223372036854775807]``. Nếu vượt quá các giới hạn đó, giá trị sẽ bị quay vòng. Nếu bạn chỉ cần đóng gói chặt chẽ các số nguyên 32-bit, hãy xem :ref:`PackedInt32Array<class_PackedInt32Array>` để có một lựa chọn tiết kiệm bộ nhớ hơn.

\ **Sự khác biệt giữa mảng đóng gói, mảng có kiểu và mảng không kiểu:** Nhìn chung, mảng đóng gói có tốc độ lặp và sửa đổi nhanh hơn so với mảng có kiểu cùng loại (ví dụ: **PackedInt64Array** so với ``Array[int]``). Ngoài ra, mảng đóng gói sử dụng ít bộ nhớ hơn. Đổi lại, mảng đóng gói kém linh hoạt hơn vì không cung cấp nhiều phương thức tiện ích như :ref:`Array.map()<class_Array_method_map>`. Mảng có kiểu lại có tốc độ lặp và sửa đổi nhanh hơn mảng không kiểu.

\ **Lưu ý:** Mảng đóng gói luôn được truyền bằng tham chiếu. Để lấy một bản sao của mảng có thể được sửa đổi độc lập với mảng ban đầu, hãy sử dụng :ref:`duplicate()<class_PackedInt64Array_method_duplicate>`. Điều này *không* áp dụng cho các thuộc tính và phương thức tích hợp sẵn. Trong những trường hợp này, mảng đóng gói được trả về là một bản sao, và việc thay đổi nó sẽ *không* ảnh hưởng đến giá trị ban đầu. Để cập nhật một thuộc tính tích hợp sẵn thuộc kiểu này, hãy sửa đổi mảng được trả về rồi gán lại mảng đó cho thuộc tính.

\ **Lưu ý:** Trong ngữ cảnh boolean, một mảng đóng gói sẽ được đánh giá là ``false`` nếu nó rỗng. Nếu không, một mảng đóng gói luôn được đánh giá là ``true``.
...
Tạo một **PackedInt64Array** dưới dạng bản sao của **PackedInt64Array** đã cho.
...
Tạo một **PackedInt64Array** mới. Bạn có thể tùy chọn truyền vào một :ref:`Array<class_Array>` tổng quát để chuyển đổi.
...
Thêm một phần tử vào cuối mảng (bí danh của :ref:`push_back()<class_PackedInt64Array_method_push_back>`).
...
Thêm một **PackedInt64Array** vào cuối mảng này.
...
Tìm chỉ mục của một giá trị hiện có (hoặc chỉ mục chèn duy trì thứ tự sắp xếp nếu giá trị chưa có trong mảng) bằng tìm kiếm nhị phân. Bạn có thể tùy chọn truyền vào một chỉ định ``before``. Nếu ``false``, chỉ mục được trả về nằm sau tất cả các mục hiện có của giá trị đó trong mảng.

\ **Lưu ý:** Gọi :ref:`bsearch()<class_PackedInt64Array_method_bsearch>` trên một mảng chưa được sắp xếp sẽ dẫn đến hành vi không mong muốn.
...
Xóa mảng. Điều này tương đương với việc sử dụng :ref:`resize()<class_PackedInt64Array_method_resize>` với kích thước ``0``.
...
Xóa lần xuất hiện đầu tiên của một giá trị khỏi mảng và trả về ``true``. Nếu giá trị không tồn tại trong mảng, không có gì xảy ra và ``false`` được trả về. Để xóa một phần tử theo chỉ mục, hãy sử dụng :ref:`remove_at()<class_PackedInt64Array_method_remove_at>`.
...
Gán giá trị đã cho cho tất cả các phần tử trong mảng. Thông thường, bạn có thể dùng phương thức này cùng với :ref:`resize()<class_PackedInt64Array_method_resize>` để tạo một mảng có kích thước nhất định và các phần tử đã được khởi tạo.
...
Tìm kiếm một giá trị trong mảng và trả về chỉ mục của giá trị đó hoặc ``-1`` nếu không tìm thấy. Bạn có thể tùy chọn truyền vào chỉ mục bắt đầu tìm kiếm.
...
Trả về số nguyên 64-bit tại ``index`` đã cho trong mảng. Nếu ``index`` nằm ngoài phạm vi hoặc là số âm, phương thức này sẽ thất bại và trả về ``0``.
...
Chèn một số nguyên mới vào vị trí đã cho trong mảng. Vị trí này phải hợp lệ hoặc nằm ở cuối mảng (``idx == size()``).
...
Đặt kích thước của mảng. Nếu mảng được mở rộng, các phần tử sẽ được dự trữ ở cuối mảng. Nếu mảng bị thu nhỏ, mảng sẽ được cắt ngắn về kích thước mới. Gọi :ref:`resize()<class_PackedInt64Array_method_resize>` một lần rồi gán các giá trị mới sẽ nhanh hơn việc thêm từng phần tử mới.

Trả về :ref:`@GlobalScope.OK<class_@GlobalScope_constant_OK>` khi thành công hoặc một trong các hằng số :ref:`Error<enum_@GlobalScope_Error>` sau đây nếu phương thức thất bại: :ref:`@GlobalScope.ERR_INVALID_PARAMETER<class_@GlobalScope_constant_ERR_INVALID_PARAMETER>` nếu kích thước là số âm hoặc :ref:`@GlobalScope.ERR_OUT_OF_MEMORY<class_@GlobalScope_constant_ERR_OUT_OF_MEMORY>` nếu việc cấp phát thất bại. Dùng :ref:`size()<class_PackedInt64Array_method_size>` để tìm kích thước thực tế của mảng sau khi resize.
...
Tìm kiếm mảng theo thứ tự ngược. Bạn có thể tùy chọn truyền vào chỉ mục bắt đầu tìm kiếm. Nếu là số âm, chỉ mục bắt đầu được tính tương đối so với cuối mảng.
...
Trả về phần lát cắt của **PackedInt64Array**, từ ``begin`` (bao gồm) đến ``end`` (không bao gồm), dưới dạng một **PackedInt64Array** mới.

Giá trị tuyệt đối của ``begin`` và ``end`` sẽ được giới hạn theo kích thước mảng, vì vậy giá trị mặc định của ``end`` khiến lát cắt mặc định kéo dài đến kích thước mảng (tức là ``arr.slice(1)`` là cách viết tắt của ``arr.slice(1, arr.size())``).
...
Trả về một bản sao của dữ liệu đã được chuyển đổi thành :ref:`PackedByteArray<class_PackedByteArray>`, trong đó mỗi phần tử được mã hóa thành 8 byte.

Kích thước của mảng mới sẽ là ``int64_array.size() * 8``.
...
Trả về một **PackedInt64Array** mới với nội dung của ``right`` được thêm vào cuối mảng này. Để có hiệu năng tốt hơn, hãy cân nhắc sử dụng :ref:`append_array()<class_PackedInt64Array_method_append_array>` thay thế.
...
Trả về ``true`` nếu nội dung của cả hai mảng giống nhau, tức là chúng có các số nguyên bằng nhau tại những chỉ mục tương ứng.
...
Trả về :ref:`int<class_int>` tại chỉ mục ``index``. Có thể sử dụng chỉ mục âm để truy cập các phần tử bắt đầu từ cuối. Việc sử dụng chỉ mục nằm ngoài giới hạn của mảng sẽ gây ra lỗi.
